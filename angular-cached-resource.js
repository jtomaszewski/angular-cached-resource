...
app.factory('$cachedResource', [
  '$resource', '$timeout', '$q', '$log', function($resource, $timeout, $q, $log) {
    ...
    return function() {
      var $key, CachedResource, Resource, actionOptions, actions, arg, args, boundParams, handler, name, param, paramDefault, paramDefaults, url, _ref;
      args = Array.prototype.slice.call(arguments);
      $key = args.shift();
      url = args.shift();
      while (args.length) {
      ...
      CachedResource = (function() {
        ...
        CachedResource._actions = actions;

        return CachedResource;

      })();
      for (name in actions) {
        actionOptions = actions[name];
        handler = actionOptions.cache != null ? Resource[name] : actionOptions.method === 'GET' && actionOptions.isArray ? readArrayCache(name, CachedResource, boundParams) : actionOptions.method === 'GET' ? readCache(name, CachedResource) : (_ref = actionOptions.method) === 'POST' || _ref === 'PUT' || _ref === 'DELETE' ? writeCache(name, CachedResource) : void 0;
        CachedResource[name] = handler;
        if (actionOptions.method !== 'GET') {
